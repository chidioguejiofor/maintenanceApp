...
export default class RequestValidator extends ModelValidator {
  constructor(model) {
    super(model, {
      ...
      title: {
        message: 'must be between 5 to 70 alphanumberic letters or puncuations and must start with a letter',
        pattern: /[A-Z][a-z0-9\s]{5,70}/i,
      },
      image: {
        message: 'must be a URI with the pattern <scheme>:\\\\<authority>',
        pattern: /.{1,}:\/\/.{1,}/,
      },
      location: {
        message: 'must be between 5 to 200 alphanumeric characters',
